...
LocalCollection.prototype.insert = function (doc, callback) {                                                // 550
  ...
  _.each(queriesToRecompute, function (qid) {                                                                // 583
    if (self.queries[qid])                                                                                   // 584
      self._recomputeResults(self.queries[qid]);                                                             // 585
  });                                                                                                        // 586
...
LocalCollection.prototype.remove = function (selector, callback) {                                           // 620
  ...
  _.each(queriesToRecompute, function (qid) {                                                                // 676
    ...
    if (query)                                                                                               // 678
      self._recomputeResults(query);                                                                         // 679
  });                                                                                                        // 680
...
LocalCollection.prototype.update = function (selector, mod, options, callback) {                             // 692
  ...
  if (!options) options = {};                                                                                // 698
                                                                                                             // 699
  var matcher = new Minimongo.Matcher(selector);                                                             // 700
                                                                                                             // 701
  ...
  _.each(self.queries, function (query, qid) {                                                               // 708
    ...
    // always an array                                                                                       // 710
    if ((query.cursor.skip || query.cursor.limit) && ! self.paused)                                          // 711
  ...
  _.each(recomputeQids, function (dummy, qid) {                                                              // 731
    ...
    if (query)                                                                                               // 733
      self._recomputeResults(query, qidToOriginalResults[qid]);                                              // 734
  });                                                                                                        // 735
...
//                                                                                                           // 921
// oldResults is guaranteed to be ignored if the query is not paused.                                        // 922
LocalCollection.prototype._recomputeResults = function (query, oldResults) {                                 // 923
  var self = this;                                                                                           // 924
  if (! self.paused && ! oldResults)                                                                         // 925
    oldResults = query.results;                                                                              // 926
  if (query.distances)                                                                                       // 927
    query.distances.clear();                                                                                 // 928
  query.results = query.cursor._getRawObjects({                                                              // 929
    ordered: query.ordered, distances: query.distances});                                                    // 930
                                                                                                             // 931
  if (! self.paused) {                                                                                       // 932
    LocalCollection._diffQueryChanges(                                                                       // 933
      query.ordered, oldResults, query.results, query);                                                      // 934
  }                                                                                                          // 935
};                                                                                                           // 936
                                                                                                             // 937
                                                                                                             // 938
LocalCollection._findInOrderedResults = function (query, doc) {                                              // 939
  if (!query.ordered)                                                                                        // 940
    throw new Error("Can't call _findInOrderedResults on unordered query");                                  // 941
  for (var i = 0; i < query.results.length; i++)                                                             // 942
    if (query.results[i] === doc)                                                                            // 943
      return i;                                                                                              // 944
  throw Error("object missing from query");                                                                  // 945
};                                                                                                           // 946
                                                                                                             // 947
// This binary search puts a value between any equal values, and the first                                   // 948
// lesser value.                                                                                             // 949
LocalCollection._binarySearch = function (cmp, array, value) {                                               // 950
  var first = 0, rangeLength = array.length;                                                                 // 951
                                                                                                             // 952
  while (rangeLength > 0) {                                                                                  // 953
    var halfRange = Math.floor(rangeLength/2);                                                               // 954
    if (cmp(value, array[first + halfRange]) >= 0) {                                                         // 955
      first += halfRange + 1;                                                                                // 956
      rangeLength -= halfRange + 1;                                                                          // 957
    } else {                                                                                                 // 958
      rangeLength = halfRange;                                                                               // 959
    }                                                                                                        // 960
  }                                                                                                          // 961
  return first;                                                                                              // 962
};                                                                                                           // 963
                                                                                                             // 964
LocalCollection._insertInSortedList = function (cmp, array, value) {                                         // 965
  if (array.length === 0) {                                                                                  // 966
    array.push(value);                                                                                       // 967
    return 0;                                                                                                // 968
  }                                                                                                          // 969
                                                                                                             // 970
  var idx = LocalCollection._binarySearch(cmp, array, value);                                                // 971
  array.splice(idx, 0, value);                                                                               // 972
  return idx;                                                                                                // 973
};                                                                                                           // 974
                                                                                                             // 975
// To track what documents are affected by a piece of code, call saveOriginals()                             // 976
// before it and retrieveOriginals() after it. retrieveOriginals returns an                                  // 977
// object whose keys are the ids of the documents that were affected since the                               // 978
// call to saveOriginals(), and the values are equal to the document's contents                              // 979
// at the time of saveOriginals. (In the case of an inserted document, undefined                             // 980
// is the value.) You must alternate between calls to saveOriginals() and                                    // 981
// retrieveOriginals().                                                                                      // 982
LocalCollection.prototype.saveOriginals = function () {                                                      // 983
  var self = this;                                                                                           // 984
  if (self._savedOriginals)                                                                                  // 985
    throw new Error("Called saveOriginals twice without retrieveOriginals");                                 // 986
  self._savedOriginals = new LocalCollection._IdMap;                                                         // 987
};                                                                                                           // 988
LocalCollection.prototype.retrieveOriginals = function () {                                                  // 989
  var self = this;                                                                                           // 990
  if (!self._savedOriginals)                                                                                 // 991
    throw new Error("Called retrieveOriginals without saveOriginals");                                       // 992
                                                                                                             // 993
  var originals = self._savedOriginals;                                                                      // 994
  self._savedOriginals = null;                                                                               // 995
  return originals;                                                                                          // 996
};                                                                                                           // 997
                                                                                                             // 998
LocalCollection.prototype._saveOriginal = function (id, doc) {                                               // 999
  var self = this;                                                                                           // 1000
  // Are we even trying to save originals?                                                                   // 1001
  if (!self._savedOriginals)                                                                                 // 1002
    return;                                                                                                  // 1003
  // Have we previously mutated the original (and so 'doc' is not actually                                   // 1004
  // original)?  (Note the 'has' check rather than truth: we store undefined                                 // 1005
  // here for inserted docs!)                                                                                // 1006
  if (self._savedOriginals.has(id))                                                                          // 1007
    return;                                                                                                  // 1008
  self._savedOriginals.set(id, EJSON.clone(doc));                                                            // 1009
};                                                                                                           // 1010
                                                                                                             // 1011
// Pause the observers. No callbacks from observers will fire until                                          // 1012
// 'resumeObservers' is called.                                                                              // 1013
LocalCollection.prototype.pauseObservers = function () {                                                     // 1014
  // No-op if already paused.                                                                                // 1015
  if (this.paused)                                                                                           // 1016
    return;                                                                                                  // 1017
                                                                                                             // 1018
  // Set the 'paused' flag such that new observer messages don't fire.                                       // 1019
  this.paused = true;                                                                                        // 1020
                                                                                                             // 1021
  // Take a snapshot of the query results for each query.                                                    // 1022
  for (var qid in this.queries) {                                                                            // 1023
    var query = this.queries[qid];                                                                           // 1024
                                                                                                             // 1025
    query.resultsSnapshot = EJSON.clone(query.results);                                                      // 1026
  }                                                                                                          // 1027
};                                                                                                           // 1028
                                                                                                             // 1029
// Resume the observers. Observers immediately receive change                                                // 1030
// notifications to bring them to the current state of the                                                   // 1031
// database. Note that this is not just replaying all the changes that                                       // 1032
// happened during the pause, it is a smarter 'coalesced' diff.                                              // 1033
LocalCollection.prototype.resumeObservers = function () {                                                    // 1034
  var self = this;                                                                                           // 1035
  // No-op if not paused.                                                                                    // 1036
  if (!this.paused)                                                                                          // 1037
    return;                                                                                                  // 1038
                                                                                                             // 1039
  // Unset the 'paused' flag. Make sure to do this first, otherwise                                          // 1040
  // observer methods won't actually fire when we trigger them.                                              // 1041
  this.paused = false;                                                                                       // 1042
                                                                                                             // 1043
  for (var qid in this.queries) {                                                                            // 1044
    var query = self.queries[qid];                                                                           // 1045
    // Diff the current results against the snapshot and send to observers.                                  // 1046
    // pass the query object for its observer callbacks.                                                     // 1047
    LocalCollection._diffQueryChanges(                                                                       // 1048
      query.ordered, query.resultsSnapshot, query.results, query);                                           // 1049
    query.resultsSnapshot = null;                                                                            // 1050
  }                                                                                                          // 1051
  self._observeQueue.drain();                                                                                // 1052
};                                                                                                           // 1053
                                                                                                             // 1054
                                                                                                             // 1055
// NB: used by livedata                                                                                      // 1056
LocalCollection._idStringify = function (id) {                                                               // 1057
  if (id instanceof LocalCollection._ObjectID) {                                                             // 1058
    return id.valueOf();                                                                                     // 1059
  } else if (typeof id === 'string') {                                                                       // 1060
    if (id === "") {                                                                                         // 1061
      return id;                                                                                             // 1062
    } else if (id.substr(0, 1) === "-" || // escape previously dashed strings                                // 1063
               id.substr(0, 1) === "~" || // escape escaped numbers, true, false                             // 1064
               LocalCollection._looksLikeObjectID(id) || // escape object-id-form strings                    // 1065
               id.substr(0, 1) === '{') { // escape object-form strings, for maybe implementing later        // 1066
      return "-" + id;                                                                                       // 1067
    } else {                                                                                                 // 1068
      return id; // other strings go through unchanged.                                                      // 1069
    }                                                                                                        // 1070
  } else if (id === undefined) {                                                                             // 1071
    return '-';                                                                                              // 1072
  } else if (typeof id === 'object' && id !== null) {                                                        // 1073
    throw new Error("Meteor does not currently support objects other than ObjectID as ids");                 // 1074
  } else { // Numbers, true, false, null                                                                     // 1075
    return "~" + JSON.stringify(id);                                                                         // 1076
  }                                                                                                          // 1077
};                                                                                                           // 1078
                                                                                                             // 1079
                                                                                                             // 1080
// NB: used by livedata                                                                                      // 1081
LocalCollection._idParse = function (id) {                                                                   // 1082
  if (id === "") {                                                                                           // 1083
    return id;                                                                                               // 1084
  } else if (id === '-') {                                                                                   // 1085
    return undefined;                                                                                        // 1086
  } else if (id.substr(0, 1) === '-') {                                                                      // 1087
    return id.substr(1);                                                                                     // 1088
  } else if (id.substr(0, 1) === '~') {                                                                      // 1089
    return JSON.parse(id.substr(1));                                                                         // 1090
  } else if (LocalCollection._looksLikeObjectID(id)) {                                                       // 1091
    return new LocalCollection._ObjectID(id);                                                                // 1092
  } else {                                                                                                   // 1093
    return id;                                                                                               // 1094
  }                                                                                                          // 1095
};                                                                                                           // 1096
                                                                                                             // 1097
LocalCollection._makeChangedFields = function (newDoc, oldDoc) {                                             // 1098
  var fields = {};                                                                                           // 1099
  LocalCollection._diffObjects(oldDoc, newDoc, {                                                             // 1100
    leftOnly: function (key, value) {                                                                        // 1101
      fields[key] = undefined;                                                                               // 1102
    },                                                                                                       // 1103
    rightOnly: function (key, value) {                                                                       // 1104
      fields[key] = value;                                                                                   // 1105
    },                                                                                                       // 1106
    both: function (key, leftValue, rightValue) {                                                            // 1107
      if (!EJSON.equals(leftValue, rightValue))                                                              // 1108
        fields[key] = rightValue;                                                                            // 1109
    }                                                                                                        // 1110
  });                                                                                                        // 1111
  return fields;                                                                                             // 1112
};                                                                                                           // 1113
...
LocalCollection._modify = function (doc, mod, options) {                                                     // 13
  ...
  _.each(newDoc, function (v, k) {                                                                           // 75
    doc[k] = v;                                                                                              // 76
  });                                                                                                        // 77
};                                                                                                           // 78
                                                                                                             // 79
// for a.b.c.2.d.e, keyparts should be ['a', 'b', 'c', '2', 'd', 'e'],                                       // 80
// and then you would operate on the 'e' property of the returned                                            // 81
// object.                                                                                                   // 82
//                                                                                                           // 83
// if options.noCreate is falsey, creates intermediate levels of                                             // 84
...
// options.noCreate is true, return undefined instead.                                                       // 87
//                                                                                                           // 88
// may modify the last element of keyparts to signal to the caller that it needs                             // 89
// to use a different value to index into the returned object (for example,                                  // 90
// ['a', '01'] -> ['a', 1]).                                                                                 // 91
//                                                                                                           // 92
// if forbidArray is true, return null if the keypath goes through an array.                                 // 93
//                                                                                                           // 94
// if options.arrayIndices is set, use its first element for the (first) '$' in                              // 95
// the path.                                                                                                 // 96
...
var MODIFIERS = {                                                                                            // 174
  ...
  $set: function (target, field, arg) {                                                                      // 186
    ...
    target[field] = EJSON.clone(arg);                                                                        // 197
  },                                                                                                         // 198
  $setOnInsert: function (target, field, arg) {                                                              // 199
    // converted to `$set` in `_modify`                                                                      // 200
  },                                                                                                         // 201
  $unset: function (target, field, arg) {                                                                    // 202
    if (target !== undefined) {                                                                              // 203
      if (target instanceof Array) {                                                                         // 204
        if (field in target)                                                                                 // 205
          target[field] = null;                                                                              // 206
      } else                                                                                                 // 207
        delete target[field];                                                                                // 208
    }                                                                                                        // 209
  },                                                                                                         // 210
  $push: function (target, field, arg) {                                                                     // 211
    if (target[field] === undefined)                                                                         // 212
      target[field] = [];                                                                                    // 213
    if (!(target[field] instanceof Array))                                                                   // 214
      throw MinimongoError("Cannot apply $push modifier to non-array");                                      // 215
                                                                                                             // 216
    if (!(arg && arg.$each)) {                                                                               // 217
      // Simple mode: not $each                                                                              // 218
      target[field].push(EJSON.clone(arg));                                                                  // 219
      return;                                                                                                // 220
    }                                                                                                        // 221
                                                                                                             // 222
    // Fancy mode: $each (and maybe $slice and $sort)                                                        // 223
    var toPush = arg.$each;                                                                                  // 224
    if (!(toPush instanceof Array))                                                                          // 225
      throw MinimongoError("$each must be an array");                                                        // 226
                                                                                                             // 227
    // Parse $slice.                                                                                         // 228
    var slice = undefined;                                                                                   // 229
    if ('$slice' in arg) {                                                                                   // 230
      if (typeof arg.$slice !== "number")                                                                    // 231
        throw MinimongoError("$slice must be a numeric value");                                              // 232
      // XXX should check to make sure integer                                                               // 233
      if (arg.$slice > 0)                                                                                    // 234
        throw MinimongoError("$slice in $push must be zero or negative");                                    // 235
      slice = arg.$slice;                                                                                    // 236
    }                                                                                                        // 237
                                                                                                             // 238
    // Parse $sort.                                                                                          // 239
    var sortFunction = undefined;                                                                            // 240
    if (arg.$sort) {                                                                                         // 241
      if (slice === undefined)                                                                               // 242
        throw MinimongoError("$sort requires $slice to be present");                                         // 243
      // XXX this allows us to use a $sort whose value is an array, but that's                               // 244
      // actually an extension of the Node driver, so it won't work                                          // 245
      // server-side. Could be confusing!                                                                    // 246
      // XXX is it correct that we don't do geo-stuff here?                                                  // 247
      sortFunction = new Minimongo.Sorter(arg.$sort).getComparator();                                        // 248
      for (var i = 0; i < toPush.length; i++) {                                                              // 249
        if (LocalCollection._f._type(toPush[i]) !== 3) {                                                     // 250
          throw MinimongoError("$push like modifiers using $sort " +                                         // 251
                      "require all elements to be objects");                                                 // 252
        }                                                                                                    // 253
      }                                                                                                      // 254
    }                                                                                                        // 255
                                                                                                             // 256
    // Actually push.                                                                                        // 257
    for (var j = 0; j < toPush.length; j++)                                                                  // 258
      target[field].push(EJSON.clone(toPush[j]));                                                            // 259
                                                                                                             // 260
    // Actually sort.                                                                                        // 261
    if (sortFunction)                                                                                        // 262
      target[field].sort(sortFunction);                                                                      // 263
                                                                                                             // 264
    // Actually slice.                                                                                       // 265
    if (slice !== undefined) {                                                                               // 266
      if (slice === 0)                                                                                       // 267
        target[field] = [];  // differs from Array.slice!                                                    // 268
      else                                                                                                   // 269
        target[field] = target[field].slice(slice);                                                          // 270
    }                                                                                                        // 271
  },                                                                                                         // 272
  $pushAll: function (target, field, arg) {                                                                  // 273
    if (!(typeof arg === "object" && arg instanceof Array))                                                  // 274
      throw MinimongoError("Modifier $pushAll/pullAll allowed for arrays only");                             // 275
    var x = target[field];                                                                                   // 276
    if (x === undefined)                                                                                     // 277
      target[field] = arg;                                                                                   // 278
    else if (!(x instanceof Array))                                                                          // 279
      throw MinimongoError("Cannot apply $pushAll modifier to non-array");                                   // 280
    else {                                                                                                   // 281
      for (var i = 0; i < arg.length; i++)                                                                   // 282
        x.push(arg[i]);                                                                                      // 283
    }                                                                                                        // 284
  },                                                                                                         // 285
  $addToSet: function (target, field, arg) {                                                                 // 286
    var isEach = false;                                                                                      // 287
    if (typeof arg === "object") {                                                                           // 288
      //check if first key is '$each'                                                                        // 289
      for (var k in arg) {                                                                                   // 290
        if (k === "$each")                                                                                   // 291
          isEach = true;                                                                                     // 292
        break;                                                                                               // 293
      }                                                                                                      // 294
    }                                                                                                        // 295
    var values = isEach ? arg["$each"] : [arg];                                                              // 296
    var x = target[field];                                                                                   // 297
    if (x === undefined)                                                                                     // 298
      target[field] = values;                                                                                // 299
    else if (!(x instanceof Array))                                                                          // 300
      throw MinimongoError("Cannot apply $addToSet modifier to non-array");                                  // 301
    else {                                                                                                   // 302
      _.each(values, function (value) {                                                                      // 303
        for (var i = 0; i < x.length; i++)                                                                   // 304
          if (LocalCollection._f._equal(value, x[i]))                                                        // 305
            return;                                                                                          // 306
        x.push(EJSON.clone(value));                                                                          // 307
      });                                                                                                    // 308
    }                                                                                                        // 309
  },                                                                                                         // 310
  $pop: function (target, field, arg) {                                                                      // 311
    if (target === undefined)                                                                                // 312
      return;                                                                                                // 313
    var x = target[field];                                                                                   // 314
    if (x === undefined)                                                                                     // 315
      return;                                                                                                // 316
    else if (!(x instanceof Array))                                                                          // 317
      throw MinimongoError("Cannot apply $pop modifier to non-array");                                       // 318
    else {                                                                                                   // 319
      if (typeof arg === 'number' && arg < 0)                                                                // 320
        x.splice(0, 1);                                                                                      // 321
      else                                                                                                   // 322
        x.pop();                                                                                             // 323
    }                                                                                                        // 324
  },                                                                                                         // 325
  $pull: function (target, field, arg) {                                                                     // 326
    if (target === undefined)                                                                                // 327
      return;                                                                                                // 328
    var x = target[field];                                                                                   // 329
    if (x === undefined)                                                                                     // 330
      return;                                                                                                // 331
    else if (!(x instanceof Array))                                                                          // 332
      throw MinimongoError("Cannot apply $pull/pullAll modifier to non-array");                              // 333
    else {                                                                                                   // 334
      var out = [];                                                                                          // 335
      if (typeof arg === "object" && !(arg instanceof Array)) {                                              // 336
        // XXX would be much nicer to compile this once, rather than                                         // 337
        // for each document we modify.. but usually we're not                                               // 338
        // modifying that many documents, so we'll let it slide for                                          // 339
        // now                                                                                               // 340
                                                                                                             // 341
        // XXX Minimongo.Matcher isn't up for the job, because we need                                       // 342
        // to permit stuff like {$pull: {a: {$gt: 4}}}.. something                                           // 343
        // like {$gt: 4} is not normally a complete selector.                                                // 344
        // same issue as $elemMatch possibly?                                                                // 345
        var matcher = new Minimongo.Matcher(arg);                                                            // 346
        for (var i = 0; i < x.length; i++)                                                                   // 347
          if (!matcher.documentMatches(x[i]).result)                                                         // 348
            out.push(x[i]);                                                                                  // 349
      } else {                                                                                               // 350
        for (var i = 0; i < x.length; i++)                                                                   // 351
          if (!LocalCollection._f._equal(x[i], arg))                                                         // 352
            out.push(x[i]);                                                                                  // 353
      }                                                                                                      // 354
      target[field] = out;                                                                                   // 355
    }                                                                                                        // 356
  },                                                                                                         // 357
  $pullAll: function (target, field, arg) {                                                                  // 358
    if (!(typeof arg === "object" && arg instanceof Array))                                                  // 359
      throw MinimongoError("Modifier $pushAll/pullAll allowed for arrays only");                             // 360
    if (target === undefined)                                                                                // 361
      return;                                                                                                // 362
    var x = target[field];                                                                                   // 363
    if (x === undefined)                                                                                     // 364
      return;                                                                                                // 365
    else if (!(x instanceof Array))                                                                          // 366
      throw MinimongoError("Cannot apply $pull/pullAll modifier to non-array");                              // 367
    else {                                                                                                   // 368
      var out = [];                                                                                          // 369
      for (var i = 0; i < x.length; i++) {                                                                   // 370
        var exclude = false;                                                                                 // 371
        for (var j = 0; j < arg.length; j++) {                                                               // 372
          if (LocalCollection._f._equal(x[i], arg[j])) {                                                     // 373
            exclude = true;                                                                                  // 374
            break;                                                                                           // 375
          }                                                                                                  // 376
        }                                                                                                    // 377
      ...
      target[field] = out;                                                                                   // 381
    }                                                                                                        // 382
  },                                                                                                         // 383
  $rename: function (target, field, arg, keypath, doc) {                                                     // 384
    if (keypath === arg)                                                                                     // 385
      // no idea why mongo has this restriction..                                                            // 386
      throw MinimongoError("$rename source must differ from target");                                        // 387
    if (target === null)                                                                                     // 388
      throw MinimongoError("$rename source field invalid");                                                  // 389
    if (typeof arg !== "string")                                                                             // 390
      throw MinimongoError("$rename target must be a string");                                               // 391
    if (target === undefined)                                                                                // 392
      return;                                                                                                // 393
    var v = target[field];                                                                                   // 394
    delete target[field];                                                                                    // 395
                                                                                                             // 396
    var keyparts = arg.split('.');                                                                           // 397
    var target2 = findModTarget(doc, keyparts, {forbidArray: true});                                         // 398
    if (target2 === null)                                                                                    // 399
      throw MinimongoError("$rename target field invalid");                                                  // 400
    var field2 = keyparts.pop();                                                                             // 401
    target2[field2] = v;                                                                                     // 402
  },                                                                                                         // 403
  $bit: function (target, field, arg) {                                                                      // 404
    // XXX mongo only supports $bit on integers, and we only support                                         // 405
    // native javascript numbers (doubles) so far, so we can't support $bit                                  // 406
    throw MinimongoError("$bit is not supported");                                                           // 407
  }                                                                                                          // 408
};                                                                                                           // 409
                                                                                                             // 410
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
