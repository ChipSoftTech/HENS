...
PV.validatePackageName = function (packageName, options) {                                           // 363
  ...
  if (packageName[0] === '.') {                                                                      // 382
    throwVersionParserError("Package names may not begin with a dot.");                              // 383
                                                                                                     ...
                                                                                                     // 386
var throwVersionParserError = function (message) {                                                   // 387
  var e = new Error(message);                                                                        // 388
  e.versionParserError = true;                                                                       // 389
  throw e;                                                                                           // 390
};                                                                                                   // 391
                                                                                                     // 392
// Return true if the version constraint was invalid prior to 0.9.3                                  // 393
// (adding _ and || support)                                                                         // 394
//                                                                                                   // 395
// NOTE: this is not used on the client yet. This package is used by the                             // 396
// package server to determine what is valid.                                                        // 397
PV.invalidFirstFormatConstraint = function (validConstraint) {                                       // 398
  if (!validConstraint) return false;                                                                // 399
  // We can check this easily right now, because we introduced some new                              // 400
  // characters. Anything with those characters is invalid prior to                                  // 401
  // 0.9.3. XXX: If we ever have to go through these, we should write a more                         // 402
  // complicated regex.                                                                              // 403
  return (/_/.test(validConstraint) ||                                                               // 404
          /\|/.test(validConstraint));                                                               // 405
};                                                                                                   // 406
                                                                                                     // 407
// Remove a suffix like "+foo" if present.                                                           // 408
PV.removeBuildID = function (versionString) {                                                        // 409
  return versionString.replace(/\+.*$/, '');                                                         // 410
};                                                                                                   // 411
///////////////////////////////////////////////////////////////////////////////////////////////////////
